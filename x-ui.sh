...
update() {
    confirm "This function will update all B3UI components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/sugitan-byte/B3UI/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    confirm "This will update B3UI to the latest DEV commit (the rolling 'dev-latest' build, not a stable release). Your data is preserved. Continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/sugitan-byte/B3UI/main/update.sh)
    if [[ $? == 0 ]]; then
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/sugitan-byte/B3UI/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/sugitan-byte/B3UI/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/sugitan-byte/B3UI/main/x-ui.sh"
    fi
}
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/sugitan-byte/B3UI/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/sugitan-byte/B3UI/main/install.sh)${plain}"
    echo ""
...
pg_ensure_hba_password_auth() {
    ...
    tmp=$(mktemp) || return 1
    {
        echo "# Added by B3UI: allow password logins for the panel database."
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│  ${green}B3UI Panel Management Script (Ads Support)${plain}  │
